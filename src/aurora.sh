...
install_nerd_font() {
	...
	if fc-list 2>/dev/null | grep -qi "FiraCode.*Nerd"; then
		gum style --foreground "$UI_ACCENT" "  $SYM_CHOCOLATE FiraCode Nerd Font já está instalada"
		return 0
	fi
	...
	while [[ $attempt -le 3 ]]; do
		if curl -fsSL --connect-timeout 10 "$font_url" -o "$cache_dir/FiraCode.zip" 2>/dev/null; then
			unzip -oq "$cache_dir/FiraCode.zip" -d "$cache_dir" 2>/dev/null || true
			cp "$cache_dir"/*.ttf "$FONT_DIR/" 2>/dev/null || true
			fc-cache -fv >/dev/null 2>&1
			rm -rf "$cache_dir"
			gum style --foreground "$UI_SUCCESS" "  $SYM_SUCCESS FiraCode Nerd Font instalada"
			return 0
		else
			gum style --foreground "$UI_WARNING" "  $SYM_WARN Tentativa $attempt falhou, tentando novamente em ${delay}s..."
			sleep $delay
			delay=$((delay * 2))
			((attempt++))
		fi
	...
	# Fallback: tentar URL alternativa (arquivo único)
	local fallback_url="https://github.com/ryanoasis/nerd-fonts/raw/master/patched-fonts/FiraCode/Regular/FiraCodeNerdFont-Regular.ttf"
	if curl -fsSL --connect-timeout 15 "$fallback_url" -o "$font_file" 2>/dev/null; then
		fc-cache -fv >/dev/null 2>&1
		rm -rf "$cache_dir"
		gum style --foreground "$UI_SUCCESS" "  $SYM_SUCCESS FiraCode Nerd Font instalada (fallback)"
		return 0
	fi

	rm -rf "$cache_dir"
	gum style --foreground "$UI_ERROR" "  $SYM_ERROR Falha ao instalar FiraCode Nerd Font após 3 tentativas"
	return 1
}
...
# Paleta de 16 cores (baseada em Ganache)
color-0=11,7,4        # Black (Deep Dark)
color-1=176,58,36     # Red
color-2=38,160,72     # Green
color-3=185,118,29    # Yellow/Brown
color-4=33,113,181    # Blue
color-5=137,62,178    # Magenta
color-6=40,142,185    # Cyan
color-7=190,173,162   # White (Roasted Almond)
color-8=106,73,40     # Bright Black (Caramel)
color-9=255,85,85     # Bright Red
color-10=85,255,85    # Bright Green
color-11=222,214,209  # Bright Yellow (Milk Foam)
color-12=85,85,255    # Bright Blue
color-13=255,85,255   # Bright Magenta
color-14=85,255,255   # Bright Cyan
color-15=239,235,232  # Bright White (White Chocolate)
...
KMSCON_EOF
	...
	gum style --foreground "$UI_SUCCESS" "  ✅ kmscon configurado"
}
...
create_systemd_service() {
	...
	cat >/etc/systemd/system/aurora-kmscon.service <<'SYSTEMD_EOF'
[Unit]
...
SYSTEMD_EOF
	...
	gum style --foreground "$UI_SUCCESS" "  $SYM_SUCCESS Serviço systemd configurado"
}
